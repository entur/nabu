...
public class EntityTypeMapper implements DTOMapper<EntityType, EntityTypeDTO> {
	...
	protected void mergeClassifications(EntityTypeDTO dto, EntityType entity) {
		...
		for (TypeDTO dtoClassification : dto.classifications) {
			if (dtoClassification.id != null) {
				EntityClassification classification = entity.getClassification(dtoClassification.id);
				removedClassifications.remove(classification);
				classificationTypeMapper.updateFromDTO(dtoClassification, classification);
			} else {
