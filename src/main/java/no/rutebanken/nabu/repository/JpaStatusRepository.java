...
public class JpaStatusRepository implements StatusRepository, DbStatus {
    ...
    @Override
    public List<Status> getLatestDeliveryStatusForProvider(Long providerId) {

        return this.entityManager.createQuery("select s1 from Status s1 where s1.correlationId= " +
                                                      "(select s2.correlationId from Status s2 where s2.action=:action and s2.date=" +
                                                      "(select max(s3.date) from Status s3 where s3.providerId=:providerId and s3.action=:action and s3.fileName not like :fileNameExcludePattern))")
                       .setParameter("action", Status.Action.FILE_TRANSFER)
