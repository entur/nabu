...
public class JpaSystemStatusRepositoryTest {
	...
	public void testAdd() throws Exception {
		SystemStatus input = new SystemStatus("4444444", SystemStatus.Action.BUILD_GRAPH, SystemStatus.State.OK, "entity", now);
	...
	public void testGetSystemStatusWithAllCriteria() throws Exception {
		SystemStatus s1 = new SystemStatus("1234567", SystemStatus.Action.BUILD_GRAPH, SystemStatus.State.OK, "entity1", now);
		repository.add(s1);
		SystemStatus s2 = new SystemStatus("1234567", SystemStatus.Action.BUILD_GRAPH, SystemStatus.State.OK, "entity1", DateUtils.addMinutes(now, 1));
		repository.add(s2);
		SystemStatus s3 = new SystemStatus("12345678", SystemStatus.Action.BUILD_GRAPH, SystemStatus.State.OK, "entity2", now);
		repository.add(s3);
		...
		Collection<SystemStatus> statusesQueryMatchingS1andS3 = repository.getSystemStatus(now, now, Arrays.asList(SystemStatus.Action.BUILD_GRAPH, SystemStatus.Action.EXPORT),
				Arrays.asList(SystemStatus.State.OK, SystemStatus.State.TIMEOUT), Arrays.asList("entity1", "entity2"));
